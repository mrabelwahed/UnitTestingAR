...
class GetUserProfileTest {
    @Test
    fun `Given call getProfileDataAsync() then the profile should be constructed correctly`() =
        runTest {
            // GIVEN
            val fakeRepo = FakeUserRepo()
            val useCase = GetUserProfile(
                fakeRepo,
            ...
            // THEN
            assertEquals(
                TestingUtils.dummyProfileData,
                result
            )
        }

    @Test
    fun `Given  call getProfileDataAsync() then I should expect concurrent calling`() =
        runTest {
            // GIVEN
    ...
    @Test
    fun `Given stable server  when call getProfileDataSync() then I should expect sequential calling`() =
        runTest {
            // GIVEN
            val fakeRepo = FakeUserRepoV2()
            val useCase = GetUserProfile(
